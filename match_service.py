...
class MatchService:    
    ...
    def match_check(data):
        ...
        for rule in rules:
            if PokemonMatcher._check_rule(rule, pokemon):
    ...
    def _check_rule(rule, pokemon):
        print('_check_role ', role, pokemon)
        match = rule['match']
        ...
        for condition in match:
            ...
            if not PokemonMatcher._check_condition(condition, pokemon):
                all_conditions_met = False
                break
        
        return all_conditions_met

    def _check_condition(condition, pokemon):
        print('_check_condition ', condition, pokemon)
        if '==' in condition:
            return PokemonMatcher._check_equality(condition, pokemon)
        elif '!=' in condition:
            return PokemonMatcher._check_inequality(condition, pokemon)
        elif '>' in condition:
            return PokemonMatcher._check_greater_than(condition, pokemon)
        elif '<' in condition:
            return PokemonMatcher._check_less_than(condition, pokemon)
        else:
            print(f"Unknown condition: {condition}")
            return False

    def _check_equality(condition, pokemon):
        print('_check_equality ', condition, pokemon)
        key, value = PokemonMatcher._extract_key_value(condition, '==')
        if value.isnumeric():
            value = int(value)
    ...
    def _check_inequality(condition, pokemon):
        print('_check_inequality ', condition, pokemon)
    ...
    def _check_greater_than(condition, pokemon):
        print('_check_greater_than ', condition, pokemon)
        key, value = PokemonMatcher._extract_key_value(condition, '>')
        if value.isnumeric():
            value = int(value)
        print(f"Comparing {key}: pokemon.get({key}) = {pokemon.get(key)}, value = {value}")
    ...
    def _check_less_than(condition, pokemon):
        print('_check_less_than ', condition, pokemon)
        key, value = PokemonMatcher._extract_key_value(condition, '<')
        if value.isnumeric():
            value = int(value)
        print(f"Comparing {key}: pokemon.get({key}) = {pokemon.get(key)}, value = {value}")
    ...
    def _extract_key_value(condition, operator):
        print('_extract_key_value ', condition, pokemon)
